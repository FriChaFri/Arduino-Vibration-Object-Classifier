...
def export_header(model_dir: Path, out_path: Path) -> None:
    ...
    output_classes = layers[-1]["output_dim"]
    if output_classes != len(label_classes):
        raise SystemExit(
            f"Output dimension ({output_classes}) does not match number of classes ({len(label_classes)})."
        )

    header = generate_header(feature_names, label_classes, imputer_stats, scaler_mean, scaler_scale, layers)
