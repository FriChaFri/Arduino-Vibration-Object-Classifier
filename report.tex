...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.8\linewidth]{figures/device.JPG}
...
The firmware samples the accelerometer at a very high rate, capturing short vibrations. 
The code detects impacts by checking for spikes in the acceleration, and then logs the accelerometer data slightly before the spike and for a fixed period of time after. 
The capture window is consistent and captures the entire impact.
...
The RMS energy is computed which summarizes overall vibration strength. 
...
These differences motivate the use of simple time-domain features rather than more computationally expensive spectral methods.
...
Between classes, clear differences appear in peak magnitude distribution and decay behavior, which the neural network learns to exploit.
...
The size of the neural network used in the demo was .
